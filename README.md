...
Make sure that the pin configuration file is included in `User_Setup.h`. In this file you can also configure some software settings. Like what kind of animation is being used, if sound will be muted in booting and if demo mode will be enabled (it is not supported right now). The only essential setting is one option for the animation.
